...
def footer():
    ...
    return f"<footer class='mt-16 border-t bg-white dark:bg-zinc-900 p-8 text-sm'><div class='max-w-5xl mx-auto'><b>{SLOGAN}</b><p class='text-xs text-gray-500 mt-2'>{db_type} - 100% Crypto - USDT BEP20/TRC20 + BNB + TRX - Pas de MoMo</p></div></footer>"
...
def home():
    ...
    <div class='text-center mt-16 px-4'>
        <h1 class='text-5xl md:text-6xl font-black leading-[0.9]'>Devis Closer — Faites de vos devis des contrats.</h1>
...
@app.route('/faq')
def faq():
    faq_html = """<div class=max-w-3xl mx-auto bg-white p-8 rounded-[32px] border><h1 class=text-3xl font-black>FAQ - Devis Closer — Faites de vos devis des contrats.</h1><div class=mt-8 space-y-6><div><b>1. Devis = contrat?</b><p class=text-sm text-gray-600>Oui, des que le client paye l'acompte crypto, le devis devient contrat avec date livraison.</p></div><div><b>2. Paiement crypto?</b><p class=text-sm>100% crypto: USDT BEP20/TRC20, BNB, TRX. Pas de MoMo.</p></div><div><b>3. Devise?</b><p class=text-sm>Choisissez FCFA, USD, EUR, USDT a la creation.</p></div><div><b>4. WhatsApp indicatif?</b><p class=text-sm>Selectionnez +229, +225, etc. + numero.</p></div><div><b>5. Date livraison?</b><p class=text-sm>Obligatoire, affichee sur recu.</p></div><div><b>6. Infos entreprise en haut?</b><p class=text-sm>Remplissez dans Settings.</p></div><div><b>7. Client peut payer sans compte?</b><p class=text-sm>Oui, lien public, paiement direct.</p></div><div><b>8. Slogan?</b><p class=text-sm>Devis Closer — Faites de vos devis des contrats.</p></div></div></div>"""
    return layout(faq_html)
...
def faq():
    faqs = [
        ("Qu'est-ce qu'un contrat dans DevisCloser ?", "Un devis validé et payé devient un contrat engageant entre vous et votre client, avec statut suivi et horodatage."),
        ("Comment les contrats sont-ils sécurisés avec la crypto ?", "Les paiements sont enregistrés sur la blockchain via les TXID BEP20/TRC20. Chaque transaction est vérifiable sur BscScan ou TronScan, garantissant transparence et traçabilité."),
        ("Quelle devise est supportée ?", "Nous supportons exclusivement les crypto : USDT sur BEP20 et TRC20, ainsi que BNB sur Binance Smart Chain et TRX sur Tron. Pas de devise fiat, pas de MoMo."),
        ("Quel est l'indicatif WhatsApp pour le support ?", "Le support est disponible via WhatsApp au +229 01 56 85 31 49. Nous répondons sous 24h en semaine."),
        ("Comment se fait la livraison du contrat ?", "Une fois l'acompte crypto confirmé, le devis passe en statut 'Contrat signé'. Vous recevez le document final téléchargeable depuis le dashboard."),
        ("Puis-je modifier un devis après envoi ?", "Non, une fois envoyé au client le devis est verrouillé. Vous pouvez créer une nouvelle version en dupliquant le devis."),
        ("Les contrats sont-ils légalement valables ?", "DevisCloser fournit une trace numérique et preuve de paiement blockchain. Pour une validité juridique, ajoutez vos CGU et conditions signées avec le client."),
        ("Comment retrouver un contrat ancien ?", "Tous vos contrats sont disponibles dans le dashboard sous la liste de vos devis. Filtrez par statut 'Payé' ou 'Acompte reçu' pour voir les contrats actifs.")
    ]
    items = ''.join([f"<details class='bg-white dark:bg-zinc-900 border rounded-xl p-4'><summary class='font-semibold cursor-pointer'>{q}</summary><p class='mt-2 text-sm text-gray-600 dark:text-gray-300'>{a}</p></details>" for q,a in faqs])
    return layout(f"<div class='max-w-3xl mx-auto'><div class='flex items-center gap-3 mb-6'><div class='w-10 h-10 rounded-xl bg-black dark:bg-white dark:text-black flex items-center justify-center text-white dark:text-black'><svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='currentColor' class='w-5 h-5'><path fill-rule='evenodd' d='M12 2a8 8 0 100 16 8 8 0 000-16zM11 13a1 1 0 112 0v1a1 1 0 11-2 0v-1zm1-6a1 1 0 100 2 1 1 0 000-2z' clip-rule='evenodd'/></svg></div><h1 class='text-2xl font-bold'>FAQ — Questions fréquentes</h1></div><div class='space-y-3'>{items}</div>", None)
