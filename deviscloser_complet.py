...
def html_page(title, content, user=None):
    ...
    nav = ""
    if user:
        # Synchronise le compte administrateur avant d'afficher le menu.
        # Le lien est visible si le rôle est déjà admin OU si l'email connecté
        # correspond à ADMIN_EMAIL. La route /admin reste protégée côté serveur.
        ...
        if is_admin and user.get("role") != "admin":
            try:
                ...
                app.logger.exception("ADMIN_MENU_SYNC_ERROR")
...
def ensure_admin():
    """Crée/promouvoit le compte administrateur défini par les variables Render.

    Cette fonction est idempotente : elle peut être appelée plusieurs fois sans
    recréer le compte ni modifier les autres comptes.
    """
    global ADMIN_READY
    if ADMIN_READY:
        return
