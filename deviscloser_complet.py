...
OWNER_BSC = os.environ.get("BSC_ADDR", "0xeB3e09b4F53d863dEBb0d49591597741612b6FB1")
OWNER_TRON = os.environ.get("TRON_ADDR", "THwRRQVtymKPwLdXdc7PmQvmvNaugX2cff")
SUPPORT_WHATSAPP = os.environ.get("SUPPORT_WHATSAPP", "22956853149")
...
def db_init():
    ...
    try:
        ...
        migrations = [
            # Migration critique : certaines bases existantes ont été créées
            # avant l'introduction de password_hash. Sans cette colonne,
            # toute tentative de connexion échoue même si /health fonctionne.
        ...
        # Compatibilité avec une éventuelle ancienne base utilisant une colonne
        # nommée "password". Si elle existe, on récupère sa valeur dans
        # password_hash. Les nouvelles inscriptions utilisent toujours un hash.
        cur.execute("""
            DO $$
            BEGIN
                IF EXISTS (
...
def admin_required(fn):
    @wraps(fn)
    def wrapper(*args, **kwargs):
        # Synchronise d'abord le compte défini par ADMIN_EMAIL.
        ensure_admin()
        user = current_user()
        if not user:
            flash("Connectez-vous avec le compte administrateur.", "warning")
            return redirect(url_for("connexion", next=request.path))
        # Sécurité de secours : si l'adresse connectée correspond exactement
        # à ADMIN_EMAIL, on la promeut immédiatement en administrateur.
        is_configured_admin = bool(ADMIN_EMAIL) and (user.get("email") or "").strip().lower() == ADMIN_EMAIL
        if is_configured_admin and user.get("role") != "admin":
            execute("UPDATE users SET role='admin', active=TRUE WHERE id=%s", (user["id"],))
            user["role"] = "admin"
...
def whatsapp_link(phone, text):
    ...
    if not digits:
        return "#"
    # Pour les numéros béninois saisis au format national (01XXXXXXXX),
    # WhatsApp attend le préfixe international 229.
    if digits.startswith("0") and len(digits) >= 9:
        digits = "229" + digits
    return f"https://wa.me/{digits}?text={urlquote(text, safe='')}"


def support_whatsapp_link():
    text = "Bonjour Devis Closer, je souhaite donner un avis / demander de l'aide concernant la plateforme."
    return whatsapp_link(SUPPORT_WHATSAPP, text)


def email_link(email, subject, body):
    if not (email or "").strip():
        return "#"
    return f"mailto:{email.strip()}?subject={urlquote(subject, safe='')}&body={urlquote(body, safe='')}"
...
def html_page(title, content, user=None):
    ...
    nav = ""
    if user:
        nav = f'''<a href="/dashboard">Dashboard</a><a href="/mes-devis">Mes devis</a><a href="/nouveau-devis">Nouveau devis</a><a href="/clients">Clients</a><a href="/abonnement">Abonnement</a><a href="/profil">Profil</a><a href="{support_whatsapp_link()}" target="_blank" rel="noopener">Support WhatsApp</a>'''
        is_admin = user.get("role") == "admin" or (ADMIN_EMAIL and (user.get("email") or "").strip().lower() == ADMIN_EMAIL)
        if is_admin:
            nav += '<a href="/admin">⚙️ Administration</a>'
        nav += '<a href="/deconnexion">Déconnexion</a>'
    else:
        nav = f'<a href="/tarifs">Tarifs</a><a href="/connexion">Connexion</a><a href="{support_whatsapp_link()}" target="_blank" rel="noopener">Support WhatsApp</a><a class="btn" href="/inscription">Créer un compte</a>'
    return render_template_string(BASE, title=title, content=content, nav=nav, csrf=csrf, user=user, owner_name=OWNER_NAME, support_link=support_whatsapp_link())
...
def ensure_admin():
    """Crée ou promeut le compte administrateur défini par Render.

    Idempotent : peut être appelé à chaque accès admin sans créer de doublon.
    """
    global ADMIN_READY
    if ADMIN_READY:
        return
    if not ADMIN_EMAIL or not ADMIN_PASSWORD:
        ADMIN_READY = True
        app.logger.warning("ADMIN_EMAIL ou ADMIN_PASSWORD non configuré")
        return
    try:
        user = query("SELECT id FROM users WHERE LOWER(email)=LOWER(%s) LIMIT 1", (ADMIN_EMAIL,), one=True)
        if not user:
            uid = execute(
                "INSERT INTO users(email,password_hash,full_name,company_name,role,cgu_accepted,cgu_version,cgu_accepted_at,active) VALUES(%s,%s,%s,%s,'admin',TRUE,%s,CURRENT_TIMESTAMP,TRUE) RETURNING id",
            ...
            execute("INSERT INTO payment_settings(user_id) VALUES(%s) ON CONFLICT (user_id) DO NOTHING", (uid,))
            app.logger.info("Compte administrateur créé pour %s", ADMIN_EMAIL)
        else:
            execute("UPDATE users SET role='admin', active=TRUE WHERE id=%s", (user["id"],))
            app.logger.info("Compte %s synchronisé comme administrateur", ADMIN_EMAIL)
        ADMIN_READY = True
    except Exception:
...
def mes_devis():
    ...
    quotes = query(f"SELECT q.*, c.name AS customer_name, c.email AS customer_email, c.phone AS customer_phone FROM quotes q LEFT JOIN customers c ON c.id=q.customer_id WHERE {where} ORDER BY q.id DESC", tuple(params))
    rows = "".join(f'''<tr><td><a href="/devis/{q['id']}">{esc(q['quote_number'])}</a></td><td>{esc(q['customer_name'] or 'Client')}</td><td>{esc(q['total'])} {esc(q['currency'])}</td><td><span class="status">{esc(q['status'])}</span></td><td>{esc(q['created_at'])}</td><td><a class="btn" href="/devis/{q['id']}">Ouvrir</a></td></tr>''' for q in quotes)
    return html_page("Mes devis", f'''<section><div class="topline"><div><div class="badge">MES DEVIS</div><h1>Mes devis</h1><p>Retrouvez et gérez tous vos devis créés depuis un seul endroit.</p></div><a class="btn primary" href="/nouveau-devis">+ Nouveau devis</a></div><div class="card"><div class="actions"><a class="btn" href="/mes-devis">Tous</a><a class="btn" href="/mes-devis?status=draft">Brouillons</a><a class="btn" href="/mes-devis?status=sent">Envoyés</a><a class="btn" href="/mes-devis?status=accepted">Acceptés</a><a class="btn" href="/mes-devis?status=signed">Signés</a><a class="btn" href="/mes-devis?status=paid">Payés</a></div></div><div class="card"><div class="tablewrap"><table><tr><th>Numéro</th><th>Client</th><th>Total</th><th>Statut</th><th>Créé le</th><th></th></tr>{rows or '<tr><td colspan="6">Aucun devis dans cette catégorie.</td></tr>'}</table></div></div></section>''', user)
...
def devis_detail(qid):
    ...
    message = f"Bonjour,\n\nVoici votre devis {q['quote_number']} : {public}\n\nVous pouvez le consulter, l'accepter, le signer et effectuer le paiement directement depuis ce lien.\n\nCordialement,\n{user['full_name']}"
    wa = whatsapp_link(q.get("customer_phone"), message)
    mail = email_link(q.get("customer_email"), f"Votre devis {q['quote_number']} — {user['company_name'] or user['full_name']}", message)
    ...
    email_btn = f'<a class="btn" href="{mail}">📧 Email</a>' if q.get("customer_email") else f'<a class="btn" href="/devis/{qid}/partager/email">📧 Email</a>'
    wa_btn = f'<a class="btn" href="{wa}" target="_blank" rel="noopener">💬 WhatsApp</a>' if q.get("customer_phone") else f'<a class="btn" href="/devis/{qid}/partager/whatsapp">💬 WhatsApp</a>'
    actions = f'''<form method="post" action="/devis/{qid}/envoyer"><input type="hidden" name="csrf" value="{csrf_token()}"><button class="btn primary">Marquer envoyé</button></form><a class="btn" href="{public}" target="_blank">Voir lien public</a>{wa_btn}{email_btn}<button class="btn" type="button" onclick="copyQuoteLink('{public}')">Copier le lien</button>'''
...
def quote_share_message(q, user):
    public = quote_url(q["public_token"])
    return f"Bonjour,\n\nVoici votre devis {q['quote_number']} : {public}\n\nVous pouvez le consulter, l'accepter, le signer et effectuer le paiement directement depuis ce lien.\n\nCordialement,\n{user['full_name']}"


@app.route("/devis/<int:qid>/partager/<canal>", methods=["GET", "POST"])
@login_required
def partager_devis(qid, canal):
    if canal not in ("whatsapp", "email"):
        abort(404)
    user = current_user()
    q = query("SELECT q.*, c.name AS customer_name, c.email AS customer_email, c.phone AS customer_phone FROM quotes q LEFT JOIN customers c ON c.id=q.customer_id WHERE q.id=%s AND q.user_id=%s", (qid, user["id"]), one=True)
    if not q:
        abort(404)
    message = quote_share_message(q, user)
    if canal == "whatsapp":
        if request.method == "POST":
            check_csrf()
        phone = request.form.get("phone", "").strip() if request.method == "POST" else (q.get("customer_phone") or "")
        if phone:
            execute("UPDATE quotes SET status='sent', updated_at=CURRENT_TIMESTAMP WHERE id=%s", (qid,))
            add_event(qid, "sent", "Devis partagé par WhatsApp", request.remote_addr)
            return redirect(whatsapp_link(phone, message))
        body = f'''<section class="formbox"><h1>Envoyer par WhatsApp</h1><p>Le client n'a pas encore de numéro WhatsApp enregistré pour ce devis.</p><form method="post"><input type="hidden" name="csrf" value="{csrf_token()}"><label>Numéro WhatsApp du client<input name="phone" placeholder="Ex. 01 90 00 00 00 ou +229 90 00 00 00" required></label><button class="btn primary full">Ouvrir WhatsApp</button></form></section>'''
    else:
        if request.method == "POST":
            check_csrf()
        email = request.form.get("email", "").strip() if request.method == "POST" else (q.get("customer_email") or "")
        if email:
            execute("UPDATE quotes SET status='sent', updated_at=CURRENT_TIMESTAMP WHERE id=%s", (qid,))
            add_event(qid, "sent", "Devis partagé par email", request.remote_addr)
            return redirect(email_link(email, f"Votre devis {q['quote_number']} — {user['company_name'] or user['full_name']}", message))
        body = f'''<section class="formbox"><h1>Envoyer par email</h1><p>Le client n'a pas encore d'adresse email enregistrée pour ce devis.</p><form method="post"><input type="hidden" name="csrf" value="{csrf_token()}"><label>Adresse email du client<input type="email" name="email" placeholder="client@exemple.com" required></label><button class="btn primary full">Ouvrir l'email</button></form></section>'''
    return html_page("Envoyer le devis", body, user)
...
def payment_settings():
    ...
    if request.method == "POST":
        check_csrf()
        vals = (request.form.get("momo_operator","").strip(),request.form.get("momo_number","").strip(),request.form.get("momo_name","").strip(),request.form.get("usdt_trc20","").strip(),request.form.get("usdt_bep20","").strip(),request.form.get("bnb_address","").strip(),request.form.get("tron_address","").strip(),user["id"])
        execute("INSERT INTO payment_settings(momo_operator,momo_number,momo_name,usdt_trc20,usdt_bep20,bnb_address,tron_address,user_id) VALUES(%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (user_id) DO UPDATE SET momo_operator=EXCLUDED.momo_operator,momo_number=EXCLUDED.momo_number,momo_name=EXCLUDED.momo_name,usdt_trc20=EXCLUDED.usdt_trc20,usdt_bep20=EXCLUDED.usdt_bep20,bnb_address=EXCLUDED.bnb_address,tron_address=EXCLUDED.tron_address,updated_at=CURRENT_TIMESTAMP", vals)
...
def admin_log(action, target_type=None, target_id=None, details=None):
    try:
        admin = current_user()
        if admin:
            execute("INSERT INTO admin_logs(admin_id,action,target_type,target_id,details) VALUES(%s,%s,%s,%s,%s)",
                    (admin["id"], action, target_type, target_id, details))
    except Exception:
        app.logger.exception("ADMIN_LOG_ERROR action=%s", action)


def admin_badge(label, value):
    return f'<div class="card"><small>{esc(label)}</small><div style="font-size:1.7rem;font-weight:800;margin-top:6px">{esc(value)}</div></div>'
...
def admin_dashboard():
    ...
    active_users = query("SELECT COUNT(*) AS n FROM users WHERE active=TRUE AND role<>'admin'", one=True)["n"]
    paid_subs = query("SELECT COUNT(*) AS n FROM subscriptions WHERE status='active' AND plan<>'free'", one=True)["n"]
    pending_subs = query("SELECT COUNT(*) AS n FROM subscription_payments WHERE status='pending'", one=True)["n"]
    validated_revenue = query("SELECT COALESCE(SUM(amount),0) AS n FROM subscription_payments WHERE status='validated'", one=True)["n"]
    ...
    quote_count = query("SELECT COUNT(*) AS n FROM quotes", one=True)["n"]
    accepted = query("SELECT COUNT(*) AS n FROM quotes WHERE status IN ('accepted','signed','paid')", one=True)["n"]
    signed = query("SELECT COUNT(*) AS n FROM quotes WHERE status IN ('signed','paid')", one=True)["n"]
    paid_quotes = query("SELECT COUNT(*) AS n FROM quotes WHERE status='paid'", one=True)["n"]
    conversion = (Decimal(str(accepted)) / Decimal(str(quote_count)) * Decimal("100")) if quote_count else Decimal("0")
    recent = query("SELECT p.id,p.plan,p.amount,p.payment_method,p.status,p.created_at,u.email,u.full_name FROM subscription_payments p JOIN users u ON u.id=p.user_id ORDER BY p.id DESC LIMIT 6")
    recent_html = "".join(f'<tr><td>#{p["id"]}</td><td>{esc(p["full_name"] or p["email"])}</td><td>{esc(p["plan"]).upper()}</td><td>{esc(p["amount"])} XOF</td><td>{esc(p["status"])}</td><td>{esc(p["created_at"])}</td></tr>' for p in recent) or '<tr><td colspan="6">Aucun paiement.</td></tr>'
    content = f'''
    <section>
      <div class="pagehead"><div><h1>Administration</h1><p>Centre de pilotage de Devis Closer.</p></div><a class="btn" href="/dashboard">Retour à l'application</a></div>
      <div class="stats">{admin_badge("Utilisateurs", users)}{admin_badge("Utilisateurs actifs", active_users)}{admin_badge("Abonnements payants", paid_subs)}{admin_badge("Paiements en attente", pending_subs)}</div>
      <div class="stats">{admin_badge("Revenus abonnements", f"{money(validated_revenue)} XOF")}{admin_badge("Commissions 2 %", f"{money(commissions)} XOF")}{admin_badge("Devis", quote_count)}{admin_badge("Conversion", f"{conversion.quantize(Decimal('0.1'))} %")}</div>
      <div class="grid two">
        <div class="card"><h2>Activité commerciale</h2><p>Acceptés : <b>{accepted}</b></p><p>Signés : <b>{signed}</b></p><p>Payés : <b>{paid_quotes}</b></p><a class="btn" href="/admin/devis">Voir les devis</a></div>
        <div class="card"><h2>Actions rapides</h2><div class="actions"><a class="btn primary" href="/admin/paiements">Vérifier les paiements</a><a class="btn" href="/admin/utilisateurs">Gérer les utilisateurs</a><a class="btn" href="/admin/abonnements">Abonnements</a></div></div>
      </div>
      <div class="card tablewrap"><h2>Derniers paiements d'abonnement</h2><table><tr><th>ID</th><th>Utilisateur</th><th>Plan</th><th>Montant</th><th>Statut</th><th>Date</th></tr>{recent_html}</table><p><a href="/admin/paiements">Voir tous les paiements →</a></p></div>
    </section>'''
    return html_page("Administration", content, current_user())
...
def admin_users():
    ...
    where, params = [], []
    if q:
        where.append("(LOWER(u.email) LIKE LOWER(%s) OR LOWER(COALESCE(u.full_name,'')) LIKE LOWER(%s) OR LOWER(COALESCE(u.company_name,'')) LIKE LOWER(%s))")
        like = f"%{q}%"; params += [like, like, like]
    if status == "active": where.append("u.active=TRUE")
    elif status == "suspended": where.append("u.active=FALSE")
    clause = (" WHERE " + " AND ".join(where)) if where else ""
    rows = query(f"SELECT u.id,u.email,u.full_name,u.company_name,u.role,u.active,u.created_at, COALESCE((SELECT plan FROM subscriptions s WHERE s.user_id=u.id ORDER BY s.id DESC LIMIT 1),'free') AS plan, (SELECT COUNT(*) FROM quotes q2 WHERE q2.user_id=u.id) AS quote_count FROM users u{clause} ORDER BY u.id DESC LIMIT 300", tuple(params))
    table = "".join(f'<tr><td>#{u["id"]}</td><td><a href="/admin/utilisateur/{u["id"]}">{esc(u["email"])}</a><br><small>{esc(u["full_name"])}</small></td><td>{esc(u["company_name"])}</td><td>{esc(u["plan"]).upper()}</td><td>{u["quote_count"]}</td><td>{"Actif" if u["active"] else "Suspendu"}</td><td><form method="post" action="/admin/utilisateur/{u["id"]}/toggle"><input type="hidden" name="csrf" value="{csrf_token()}"><button class="btn">{"Suspendre" if u["active"] else "Réactiver"}</button></form></td></tr>' for u in rows) or '<tr><td colspan="7">Aucun utilisateur trouvé.</td></tr>'
    content = f'''
    <section><div class="pagehead"><div><h1>Utilisateurs</h1><p>Recherche, suivi et gestion des comptes.</p></div><a class="btn" href="/admin">← Admin</a></div>
    <div class="card"><form method="get" class="formgrid"><label>Recherche<input name="q" value="{esc(q)}" placeholder="Email, nom ou entreprise"></label><label>État<select name="status"><option value="all" {"selected" if status=="all" else ""}>Tous</option><option value="active" {"selected" if status=="active" else ""}>Actifs</option><option value="suspended" {"selected" if status=="suspended" else ""}>Suspendus</option></select></label><button class="btn primary" type="submit">Rechercher</button></form></div>
    <div class="card tablewrap"><table><tr><th>ID</th><th>Utilisateur</th><th>Entreprise</th><th>Plan</th><th>Devis</th><th>État</th><th>Action</th></tr>{table}</table></div></section>'''
    return html_page("Utilisateurs", content, current_user())
...
def admin_user_detail(uid):
    ...
    if not u: abort(404)
    sub = query("SELECT * FROM subscriptions WHERE user_id=%s ORDER BY id DESC LIMIT 1", (uid,), one=True)
    quotes = query("SELECT status,COUNT(*) AS n,COALESCE(SUM(total),0) AS total FROM quotes WHERE user_id=%s GROUP BY status ORDER BY status", (uid,))
    payments = query("SELECT id,plan,amount,currency,payment_method,status,transaction_id,created_at,verified_at FROM subscription_payments WHERE user_id=%s ORDER BY id DESC LIMIT 20", (uid,))
    qrows = "".join(f'<tr><td>{esc(x["status"])}</td><td>{x["n"]}</td><td>{esc(x["total"])} {esc(u["currency"] or "XOF")}</td></tr>' for x in quotes) or '<tr><td colspan="3">Aucun devis.</td></tr>'
    prows = "".join(f'<tr><td>#{p["id"]}</td><td>{esc(p["plan"]).upper()}</td><td>{esc(p["amount"])} {esc(p["currency"] or "XOF")}</td><td>{esc(p["payment_method"])}</td><td>{esc(p["status"])}</td><td>{esc(p["created_at"])}</td></tr>' for p in payments) or '<tr><td colspan="6">Aucun paiement.</td></tr>'
    sub_html = f'<p>Plan : <b>{esc(sub["plan"]).upper()}</b></p><p>Statut : <b>{esc(sub["status"])}</b></p><p>Début : {esc(sub["started_at"])}</p><p>Expiration : {esc(sub["expires_at"])}</p>' if sub else '<p>Aucun abonnement enregistré.</p>'
    content = f'''
    <section><div class="pagehead"><div><h1>Utilisateur #{uid}</h1><p>{esc(u["email"])}</p></div><a class="btn" href="/admin/utilisateurs">← Utilisateurs</a></div>
      <div class="grid two"><div class="card"><h2>Profil</h2><p><b>Nom :</b> {esc(u["full_name"])}</p><p><b>Entreprise :</b> {esc(u["company_name"])}</p><p><b>Téléphone :</b> {esc(u["phone"])}</p><p><b>Pays :</b> {esc(u["country"])}</p><p><b>Devise :</b> {esc(u["currency"])}</p><p><b>Inscription :</b> {esc(u["created_at"])}</p><p><b>Rôle :</b> {esc(u["role"])}</p><p><b>État :</b> {"Actif" if u["active"] else "Suspendu"}</p></div>
      <div class="card"><h2>Abonnement</h2>{sub_html}<form method="post" action="/admin/utilisateur/{uid}/toggle"><input type="hidden" name="csrf" value="{csrf_token()}"><button class="btn">{"Suspendre le compte" if u["active"] else "Réactiver le compte"}</button></form></div></div>
      <div class="card tablewrap"><h2>Devis par statut</h2><table><tr><th>Statut</th><th>Nombre</th><th>Valeur</th></tr>{qrows}</table></div>
      <div class="card tablewrap"><h2>Historique des paiements d'abonnement</h2><table><tr><th>ID</th><th>Plan</th><th>Montant</th><th>Méthode</th><th>Statut</th><th>Date</th></tr>{prows}</table></div>
    </section>'''
    return html_page("Utilisateur", content, current_user())
...
def admin_toggle_user(uid):
    ...
    u = query("SELECT active FROM users WHERE id=%s", (uid,), one=True)
    if not u: abort(404)
    new_active = not bool(u["active"])
    execute("UPDATE users SET active=%s WHERE id=%s", (new_active, uid))
    admin_log("toggle_user", "user", uid, "active=" + str(new_active))
...
def admin_subscriptions():
    rows = query("SELECT s.*,u.email,u.full_name FROM subscriptions s JOIN users u ON u.id=s.user_id ORDER BY CASE WHEN s.status='active' THEN 0 ELSE 1 END, s.expires_at DESC NULLS LAST, s.id DESC LIMIT 300")
    table = "".join(f'<tr><td>#{s["id"]}</td><td><a href="/admin/utilisateur/{s["user_id"]}">{esc(s["full_name"] or s["email"])}</a><br><small>{esc(s["email"])}</small></td><td>{esc(s["plan"]).upper()}</td><td>{esc(s["status"])}</td><td>{esc(s["started_at"])}</td><td>{esc(s["expires_at"])}</td></tr>' for s in rows) or '<tr><td colspan="6">Aucun abonnement.</td></tr>'
    return html_page("Abonnements", f'<section><div class="pagehead"><div><h1>Abonnements</h1><p>Suivi des plans et des périodes d’accès.</p></div><a class="btn" href="/admin">← Admin</a></div><div class="card tablewrap"><table><tr><th>ID</th><th>Utilisateur</th><th>Plan</th><th>Statut</th><th>Début</th><th>Expiration</th></tr>{table}</table></div></section>', current_user())
...
def admin_payments():
    status = request.args.get("status", "all")
    where, params = [], []
    if status in ("pending", "validated", "rejected"):
        where.append("p.status=%s"); params.append(status)
    clause = (" WHERE " + " AND ".join(where)) if where else ""
    rows = query(f"SELECT p.*,u.email,u.full_name FROM subscription_payments p JOIN users u ON u.id=p.user_id{clause} ORDER BY CASE WHEN p.status='pending' THEN 0 ELSE 1 END,p.id DESC LIMIT 300", tuple(params))
    cards = ""
    for p in rows:
        proof = f'<a href="/admin/paiement/{p["id"]}/preuve" target="_blank">Voir la preuve</a>' if p["proof_data"] else "Pas de preuve"
        action = ''
        if p["status"] == "pending":
            action = f'<div class="actions"><form method="post" action="/admin/paiement/{p["id"]}/valider"><input type="hidden" name="csrf" value="{csrf_token()}"><button class="btn primary">Valider</button></form><form method="post" action="/admin/paiement/{p["id"]}/rejeter"><input type="hidden" name="csrf" value="{csrf_token()}"><input name="reason" placeholder="Motif du rejet"><button class="btn danger">Rejeter</button></form></div>'
        rejection = f'<p>Motif : {esc(p["rejection_reason"])}</p>' if p["rejection_reason"] else ''
        cards += f'<div class="card"><div class="pagehead"><div><h3>#{p["id"]} — {esc(p["plan"]).upper()}</h3><p>{esc(p["full_name"] or p["email"])} · {esc(p["email"])}</p></div><span class="status">{esc(p["status"])}</span></div><p><b>{esc(p["amount"])} {esc(p["currency"] or "XOF")}</b> · {esc(p["payment_method"])} · TXID : {esc(p["transaction_id"])}</p><p>Créé le {esc(p["created_at"])} · {proof}</p>{rejection}{action}</div>'
    filters = ''.join(f'<a class="btn {"primary" if status==v else ""}" href="/admin/paiements?status={v}">{label}</a>' for v,label in [("all","Tous"),("pending","En attente"),("validated","Validés"),("rejected","Rejetés")])
    return html_page("Paiements", f'<section><div class="pagehead"><div><h1>Paiements abonnements</h1><p>Contrôle manuel des paiements reçus.</p></div><a class="btn" href="/admin">← Admin</a></div><div class="actions">{filters}</div><div>{cards or "<div class=card>Aucun paiement pour ce filtre.</div>"}</div></section>', current_user())
...
def admin_validate_payment(pid):
    ...
    admin_log("validate_payment", "subscription_payment", pid, f"plan={p['plan']};amount={p['amount']}")
...
def admin_reject_payment(pid):
    check_csrf()
    reason = request.form.get("reason", "Paiement non validé").strip()[:500]
    p = query("SELECT * FROM subscription_payments WHERE id=%s", (pid,), one=True)
    if not p: abort(404)
    if p["status"] != "pending": return redirect(url_for("admin_payments"))
    ...
    admin_log("reject_payment", "subscription_payment", pid, reason)
    return redirect(url_for("admin_payments"))
...
def admin_quotes():
    ...
    q = request.args.get("q", "").strip()
    where, params = [], []
    if status != "all": where.append("q.status=%s"); params.append(status)
    if q:
        where.append("(q.quote_number ILIKE %s OR c.name ILIKE %s OR u.email ILIKE %s OR COALESCE(u.company_name,'') ILIKE %s)")
        like=f"%{q}%"; params += [like,like,like,like]
    clause=(" WHERE "+" AND ".join(where)) if where else ""
    rows=query(f"SELECT q.id,q.quote_number,q.total,q.currency,q.status,q.created_at,q.updated_at,c.name AS customer_name,u.id AS user_id,u.email,u.full_name FROM quotes q JOIN users u ON u.id=q.user_id LEFT JOIN customers c ON c.id=q.customer_id{clause} ORDER BY q.id DESC LIMIT 300",tuple(params))
    table="".join(f'<tr><td>{esc(r["quote_number"])}</td><td>{esc(r["customer_name"])}</td><td><a href="/admin/utilisateur/{r["user_id"]}">{esc(r["full_name"] or r["email"])}</a></td><td>{esc(r["total"])} {esc(r["currency"])}</td><td>{esc(r["status"])}</td><td>{esc(r["updated_at"])}</td></tr>' for r in rows) or '<tr><td colspan="6">Aucun devis.</td></tr>'
    opts=''.join(f'<option value="{v}" {"selected" if status==v else ""}>{v.capitalize()}</option>' for v in ["draft","sent","viewed","negotiation","accepted","signed","paid","refused","expired"])
    return html_page("Devis", f'<section><div class="pagehead"><div><h1>Devis</h1><p>Vue globale de l’activité commerciale.</p></div><a class="btn" href="/admin">← Admin</a></div><div class="card"><form method="get" class="formgrid"><label>Recherche<input name="q" value="{esc(q)}" placeholder="N° devis, client, utilisateur"></label><label>Statut<select name="status"><option value="all">Tous</option>{opts}</select></label><button class="btn primary">Filtrer</button></form></div><div class="card tablewrap"><table><tr><th>Devis</th><th>Client</th><th>Professionnel</th><th>Total</th><th>Statut</th><th>Mis à jour</th></tr>{table}</table></div></section>', current_user())


@app.route("/admin/transactions")
@admin_required
def admin_transactions():
    rows=query("SELECT qp.id,qp.quote_id,qp.professional_id,qp.gross_amount,qp.commission_amount,qp.net_amount,qp.currency,qp.payment_method,qp.transaction_id,qp.status,qp.created_at,u.email,u.full_name,q.quote_number FROM quote_payments qp JOIN users u ON u.id=qp.professional_id JOIN quotes q ON q.id=qp.quote_id ORDER BY qp.id DESC LIMIT 300")
    total_gross=query("SELECT COALESCE(SUM(gross_amount),0) AS n FROM quote_payments",one=True)["n"]
    total_comm=query("SELECT COALESCE(SUM(commission_amount),0) AS n FROM quote_payments WHERE status IN ('pending','verified')",one=True)["n"]
    table="".join(f'<tr><td>#{r["id"]}</td><td>{esc(r["quote_number"])}</td><td>{esc(r["full_name"] or r["email"])}</td><td>{esc(r["gross_amount"])} {esc(r["currency"] or "XOF")}</td><td>{esc(r["commission_amount"])}</td><td>{esc(r["net_amount"])}</td><td>{esc(r["payment_method"])}</td><td>{esc(r["status"])}</td><td>{esc(r["created_at"])}</td></tr>' for r in rows) or '<tr><td colspan="9">Aucune transaction.</td></tr>'
    return html_page("Transactions", f'<section><div class="pagehead"><div><h1>Paiements de devis</h1><p>Suivi des paiements directs entre clients et professionnels.</p></div><a class="btn" href="/admin">← Admin</a></div><div class="stats">{admin_badge("Volume brut",f"{money(total_gross)} XOF")}{admin_badge("Commissions 2 %",f"{money(total_comm)} XOF")}</div><div class="card"><p><b>Important :</b> Devis Closer ne détient pas les fonds des clients. Cette page enregistre les transactions déclarées et les commissions.</p></div><div class="card tablewrap"><table><tr><th>ID</th><th>Devis</th><th>Professionnel</th><th>Brut</th><th>Commission</th><th>Net</th><th>Méthode</th><th>Statut</th><th>Date</th></tr>{table}</table></div></section>', current_user())
...
def admin_notifications():
    if request.method == "POST":
        check_csrf()
        uid_raw=request.form.get("user_id","").strip(); title=request.form.get("title","").strip()[:150]; message=request.form.get("message","").strip()[:1000]
        try: uid=int(uid_raw)
        except ValueError: uid=0
        if not uid or not title or not message: flash("Utilisateur, titre et message sont obligatoires.","warning")
        elif not query("SELECT id FROM users WHERE id=%s",(uid,),one=True): flash("Utilisateur introuvable.","warning")
        else:
            notify(uid,title,message); admin_log("send_notification","user",uid,title); flash("Notification envoyée.","success")
    users=query("SELECT id,email,full_name FROM users WHERE role<>'admin' ORDER BY id DESC LIMIT 500")
    opts="".join(f'<option value="{u["id"]}">#{u["id"]} — {esc(u["full_name"] or u["email"])} — {esc(u["email"])}</option>' for u in users)
    return html_page("Notifications", f'<section><div class="pagehead"><div><h1>Notifications</h1><p>Envoyer un message interne à un utilisateur.</p></div><a class="btn" href="/admin">← Admin</a></div><div class="card"><form method="post"><input type="hidden" name="csrf" value="{csrf_token()}"><label>Utilisateur<select name="user_id" required><option value="">Choisir…</option>{opts}</select></label><label>Titre<input name="title" maxlength="150" required></label><label>Message<textarea name="message" rows="5" maxlength="1000" required></textarea></label><button class="btn primary">Envoyer</button></form></div></section>', current_user())


@app.route("/admin/journal")
@admin_required
def admin_logs():
    rows=query("SELECT l.*,u.email FROM admin_logs l LEFT JOIN users u ON u.id=l.admin_id ORDER BY l.id DESC LIMIT 300")
    table="".join(f'<tr><td>#{r["id"]}</td><td>{esc(r["email"])}</td><td>{esc(r["action"])}</td><td>{esc(r["target_type"])} #{esc(r["target_id"])}</td><td>{esc(r["details"])}</td><td>{esc(r["created_at"])}</td></tr>' for r in rows) or '<tr><td colspan="6">Aucune action enregistrée.</td></tr>'
    return html_page("Journal", f'<section><div class="pagehead"><div><h1>Journal d’administration</h1><p>Traçabilité des actions sensibles.</p></div><a class="btn" href="/admin">← Admin</a></div><div class="card tablewrap"><table><tr><th>ID</th><th>Admin</th><th>Action</th><th>Cible</th><th>Détails</th><th>Date</th></tr>{table}</table></div></section>', current_user())


@app.route("/admin/configuration")
@admin_required
def admin_configuration():
    return html_page("Configuration", f'''
    <section><div class="pagehead"><div><h1>Configuration</h1><p>Paramètres opérationnels du back-office.</p></div><a class="btn" href="/admin">← Admin</a></div>
      <div class="grid two">
        <div class="card"><h2>Plans</h2><p>Free : 0 XOF</p><p>Starter : 6 500 XOF / 30 jours</p><p>Pro : 15 000 XOF / 30 jours</p><p>Commission paiements de devis : 2 %</p></div>
        <div class="card"><h2>Paiement des abonnements</h2><p><b>Nom :</b> {esc(OWNER_NAME)}</p><p><b>MoMo :</b> {esc(OWNER_MOMO)}</p><p><b>USDT BEP20 / BSC :</b> {esc(OWNER_BSC)}</p><p><b>USDT TRC20 / TRON :</b> {esc(OWNER_TRON)}</p><p>Ces valeurs sont chargées depuis les variables d’environnement et ne sont pas modifiables depuis cette page.</p></div>
      </div>
      <div class="card"><h2>Support</h2><p>WhatsApp : <a href="{support_whatsapp_link()}" target="_blank" rel="noopener">{esc(SUPPORT_WHATSAPP)}</a></p><p>Pour modifier les coordonnées sensibles, utilise les variables d’environnement du service Render.</p></div>
    </section>''', current_user())
...
:root{--bg:#080b12;--card:#111722;--line:#273142;--text:#f4f7fb;--muted:#9ba8b8;--blue:#2684ff;--danger:#ff5c68;--ok:#25c78a;--shadow:0 20px 60px rgba(0,0,0,.22)}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 15% 0,#14233b 0,transparent 35%),var(--bg);color:var(--text);font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;line-height:1.55}a{color:#76b4ff;text-decoration:none}a:hover{text-decoration:underline}.nav{position:sticky;top:0;z-index:100;background:rgba(8,11,18,.94);backdrop-filter:blur(14px);border-bottom:1px solid var(--line)}.navin{max-width:1180px;margin:auto;display:flex;align-items:center;gap:14px;padding:14px 18px}.brand{font-weight:900;letter-spacing:-.5px;color:#fff;margin-right:auto}.brand span{color:#4c9aff}.menu-toggle{width:44px;height:44px;border:1px solid var(--line);border-radius:10px;background:#111722;color:#fff;font-size:24px;cursor:pointer;display:inline-flex;align-items:center;justify-content:center}.drawer-backdrop{display:none;position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:110}.drawer{position:fixed;top:0;left:-310px;width:min(300px,86vw);height:100vh;background:#0d131d;border-right:1px solid var(--line);z-index:120;padding:22px;box-shadow:20px 0 60px rgba(0,0,0,.35);transition:left .2s ease;overflow:auto}.drawer.open{left:0}.drawer-backdrop.open{display:block}.drawer-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:25px}.drawer-links{display:flex;flex-direction:column;gap:7px}.drawer-links a{display:block;padding:13px 12px;border-radius:10px;font-size:15px}.drawer-links a:hover{background:#151d2a;text-decoration:none}.nav a{font-size:14px}.btn{display:inline-flex;align-items:center;justify-content:center;border:1px solid var(--line);background:#151d2a;color:#fff;border-radius:10px;padding:10px 14px;cursor:pointer;text-decoration:none;gap:7px}.btn:hover{background:#1b2636;text-decoration:none}.btn.primary{background:var(--blue);border-color:var(--blue)}.btn.danger{background:transparent;border-color:var(--danger);color:#ff8a92}.full{width:100%}main{max-width:1180px;margin:0 auto;padding:42px 18px 80px}.hero{padding:70px 0;text-align:center;max-width:850px;margin:auto}.hero h1{font-size:clamp(38px,7vw,72px);line-height:1.02;letter-spacing:-3px;margin:16px 0}.hero p{font-size:20px;color:var(--muted)}.badge,.tag{display:inline-block;font-size:11px;font-weight:800;letter-spacing:1.2px;color:#8ec2ff;border:1px solid #28548b;background:#0e2038;border-radius:999px;padding:6px 10px}.tag{color:#9ef0ce;border-color:#24634e;background:#0d2a20;float:right}.actions{display:flex;flex-wrap:wrap;gap:9px;align-items:center}.grid{display:grid;gap:18px}.grid.two{grid-template-columns:repeat(2,minmax(0,1fr))}.grid.three{grid-template-columns:repeat(3,minmax(0,1fr))}.card,.formbox{background:linear-gradient(180deg,rgba(255,255,255,.03),rgba(255,255,255,.015));border:1px solid var(--line);border-radius:16px;padding:22px;box-shadow:var(--shadow)}.formbox{max-width:700px;margin:auto}.formbox.wide{max-width:900px}.center{text-align:center}.prose{max-width:900px;margin:auto}.topline{display:flex;justify-content:space-between;align-items:center;gap:20px;margin-bottom:22px}.pagehead{display:flex;justify-content:space-between;align-items:flex-start;gap:18px;margin-bottom:22px}.pagehead h1{margin:0 0 6px}.pagehead p{margin:0;color:var(--muted)}.formgrid{display:grid;grid-template-columns:2fr 1fr auto;gap:12px;align-items:end}.formgrid label{margin:0}@media(max-width:850px){.formgrid{grid-template-columns:1fr}.pagehead{flex-direction:column}}.stats{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin:20px 0}.stat{padding:18px;border:1px solid var(--line);border-radius:14px;background:#0d131d}.stat small{display:block;color:var(--muted)}.stat strong{font-size:24px;display:block;margin-top:5px}label{display:block;font-weight:700;margin:14px 0}input,select,textarea{width:100%;margin-top:7px;background:#0a1019;border:1px solid #2a3545;color:#fff;border-radius:10px;padding:12px;font:inherit}textarea{min-height:110px;resize:vertical}.check{font-weight:400}.check input{width:auto;margin-right:8px}.price{font-size:30px;font-weight:900;margin:8px 0 15px}.plan.featured{border-color:#3b8fff}.plan ul{min-height:240px;padding-left:20px;color:var(--muted)}li{margin:6px 0}.tablewrap{overflow:auto}table{width:100%;border-collapse:collapse;min-width:650px}th,td{padding:12px;border-bottom:1px solid var(--line);text-align:left}.status{display:inline-block;padding:5px 9px;border-radius:999px;background:#182538;color:#9dc8ff;font-size:12px}.totals{max-width:380px;margin:20px 0 0 auto}.itemrow{display:grid;grid-template-columns:1fr 120px 160px 45px;gap:8px;margin:8px 0}.remove{padding:8px}.muted{color:var(--muted)}code{word-break:break-all;color:#b9d7ff}.flash{padding:12px 14px;border-radius:10px;margin-bottom:12px;background:#162234;border:1px solid var(--line)}.flash.success{border-color:#24634e}.flash.danger{border-color:#6b2e35}.flash.warning{border-color:#765e27}footer{border-top:1px solid var(--line);color:var(--muted);padding:25px 18px;text-align:center}@media(max-width:850px){.grid.three,.grid.two,.stats{grid-template-columns:1fr}.topline{align-items:flex-start;flex-direction:column}.hero{padding:45px 0}.itemrow{grid-template-columns:1fr 1fr}.itemrow input:first-child{grid-column:1/-1}.itemrow .remove{grid-column:auto}}
</style></head><body><header class="nav"><div class="navin"><button class="menu-toggle" type="button" aria-label="Ouvrir le menu" onclick="openDrawer()">☰</button><a class="brand" href="/">Devis <span>Closer</span></a></div></header><div id="drawerBackdrop" class="drawer-backdrop" onclick="closeDrawer()"></div><aside id="drawer" class="drawer" aria-label="Menu principal"><div class="drawer-head"><strong>Menu</strong><button class="menu-toggle" type="button" aria-label="Fermer le menu" onclick="closeDrawer()">×</button></div><nav class="drawer-links">{{ nav|safe }}</nav></aside><main>{% with messages=get_flashed_messages(with_categories=true) %}{% for category,message in messages %}<div class="flash {{ category }}">{{ message }}</div>{% endfor %}{% endwith %}{{ content|safe }}</main><footer>Devis Closer — Transformez vos devis en contrats · <a href="/cgu">CGU</a> · <a href="/health">Statut</a> · <a href="{{ support_link }}" target="_blank" rel="noopener">Support & avis WhatsApp</a></footer><script>function openDrawer(){document.getElementById('drawer').classList.add('open');document.getElementById('drawerBackdrop').classList.add('open');}function closeDrawer(){document.getElementById('drawer').classList.remove('open');document.getElementById('drawerBackdrop').classList.remove('open');}function copyQuoteLink(link){if(navigator.clipboard){navigator.clipboard.writeText(link).then(function(){alert('Lien copié.');}).catch(function(){window.prompt('Copiez le lien du devis :',link);});}else{window.prompt('Copiez le lien du devis :',link);}}</script></body></html>'''
