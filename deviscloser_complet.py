...
def login():
    if request.method=="POST":
        ...
        if user and check_password_hash(user['password'],pwd):
            session['user_id']=user['id']; return redirect("/dashboard")
        return layout(f"""<div class='max-w-sm mx-auto bg-white dark:bg-zinc-900 p-8 rounded-[24px] mt-16 border text-center'>
            <div class='text-5xl'>❌</div>
            <h2 class='text-xl font-bold mt-4'>Email ou mot de passe incorrect</h2>
            <p class='text-sm text-gray-500 mt-2'>Base de données réinitialisée après deploy Render (plan gratuit).<br>Ton compte {email} n'existe plus.</p>
            <div class='mt-6 space-y-3'>
                <a href='/register' class='block bg-black dark:bg-white dark:text-black text-white py-3 rounded-full font-bold'>Créer un nouveau compte</a>
                <a href='/login' class='block border py-3 rounded-full font-bold'>Réessayer</a>
            </div>
            <p class='text-xs text-gray-400 mt-4'>Astuce: Sur Render gratuit, la DB s'efface à chaque deploy. Pour prod, il faut Postgres.</p>
        </div>""")
    return layout("""<div class='max-w-sm mx-auto bg-white dark:bg-zinc-900 p-6 rounded-[24px] mt-16 border'>
        <h2 class='text-xl font-bold'>Connexion</h2>
        <p class='text-xs text-gray-500 mt-1'>Si tu viens de re-déployer, re-crée ton compte (DB effacée sur Render gratuit)</p>
        <form method='POST' class='mt-4 space-y-3'>
            <input name='email' type='email' placeholder='Email' required class='border p-3 rounded-xl w-full dark:bg-black'>
            <input name='password' type='password' placeholder='Mot de passe' required class='border p-3 rounded-xl w-full dark:bg-black'>
            <button class='bg-black dark:bg-white dark:text-black text-white w-full py-3 rounded-full font-bold'>Se connecter</button>
        </form>
        <p class='text-sm text-center mt-4'><a href='/register' class='text-blue-600'>Pas de compte? S'inscrire</a></p>
        <div class='mt-4 text-center'><button onclick="toggleDark()" class='border px-4 py-1 rounded-full text-xs'>🌙/☀️ Mode sombre</button></div>
    </div>""")
