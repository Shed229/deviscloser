...
PLANS = {
    "free": {
        "name": "Free", "price": Decimal("0"), "quotes": 1, "clients": 5, "users": 1,
        "features": ["1 devis/mois", "5 clients", "WhatsApp", "Email", "Lien public", "Suivi de base", "1 modèle", "Dashboard de base"]
    },
    "starter": {
        "name": "Starter", "price": Decimal("6500"), "quotes": 50, "clients": 100, "users": 2,
        "features": ["50 devis/mois", "100 clients", "WhatsApp + email", "Suivi des devis", "Relances", "5 modèles", "Signature électronique", "Paiements de devis", "Statistiques", "Historique client", "2 utilisateurs"]
    },
    "pro": {
...
def whatsapp_link(phone, text):
    ...
    return f"https://wa.me/{digits}?text={text.replace(' ', '%20').replace('\n', '%0A')}" if digits else "#"
...
def before_request():
    if request.path != "/health":
        try:
            ...
            app.logger.exception("DB startup error: %s", exc)
            if request.path != "/health":
                # L'application reste démarrée; Render peut afficher le détail dans les logs.
                pass
...
def forgot_password():
    if request.method == "POST":
        ...
        if user:
            ...
            # Stockage léger dans une table temporaire créée à la volée.
            execute("CREATE TABLE IF NOT EXISTS password_resets(id BIGSERIAL PRIMARY KEY,user_id BIGINT REFERENCES users(id) ON DELETE CASCADE,token_hash TEXT UNIQUE,expires_at TIMESTAMP,used BOOLEAN DEFAULT FALSE)")
...
def cgu():
    ...
    <h2>3. Paiements d'abonnement</h2><p>Les abonnements Starter et Pro sont payants selon les tarifs affichés. Le paiement peut être soumis à une vérification manuelle, notamment pour MoMo et certaines cryptomonnaies.</p>
...
def server_error(e):
    ...
    return html_page("Erreur", '<section class="card center"><h1>Erreur serveur</h1><p>Une erreur interne est survenue. Consultez les logs Render.</p></section>', current_user()), 500
