...
def html_page(title, content, user=None):
    ...
    nav = ""
    if user:
        nav = f'''<a href="/dashboard">Dashboard</a><a href="/nouveau-devis">Nouveau devis</a><a href="/clients">Clients</a><a href="/abonnement">Abonnement</a><a href="/profil">Profil</a><a href="{support_whatsapp_link()}" target="_blank" rel="noopener">Support WhatsApp</a>'''
        if user["role"] == "admin":
...
def devis_detail(qid):
    ...
    email_btn = f'<a class="btn" href="{mail}">Email</a>' if q.get("customer_email") else '<span class="btn disabled">Email indisponible</span>'
    wa_btn = f'<a class="btn" href="{wa}" target="_blank" rel="noopener">WhatsApp</a>' if q.get("customer_phone") else '<span class="btn disabled">WhatsApp indisponible</span>'
    actions = f'''<form method="post" action="/devis/{qid}/envoyer"><input type="hidden" name="csrf" value="{csrf_token()}"><button class="btn primary">Marquer envoyé</button></form><a class="btn" href="{public}" target="_blank">Voir lien public</a>{wa_btn}{email_btn}<button class="btn" onclick="navigator.clipboard.writeText('{public}')">Copier le lien</button>'''
